...
现在null的 as 和 is 判断要注意有没有？，没有？都不行，有？就行

---

你是不可能在同步函数的执行过程中获取到异步（async函数，Future）的结果值的

---

?. 有短路的功能，只要第一次检查到null就会返回null，所以连写时只要第一个加?

---

要用 .. 级联，一旦第一个开始用 .. 了，后面都要用 ..

---

继承树上只要有一个实现了某函数，字类就不一定需要实现，换言之，不能通过在中间某一级将方法重写为需的进行“拦截”

---

对于非null类型的函数参数，就算有默认值，也不可显式的传入可null值，这增加了程序的安全性
