...
commonJS中包不可以循环引用，module中则可以，因为它只是生成一个引用，动态时才会去算是否有。需手动保证有。

airbnb的eslint不让循环引用module
...
Base64和字符编码utf8等，本质上都是二进制数据与字符之间的转换

---

目前常用的Hash算法为MD5，SHA1，SHA2（SHA2-XXX），SHA3，SHA2相对最安全（是SHA2的备用）。

Hash算法最基本的是通过取模来规约到固定范围，通过异或消除规律
...
它是函数定义时的关键字，不在函数定义中的话没有意义。

对于对象的方法，一般定义时里面的this指向此对象，箭头函数中的this无意义，因此定义对象方法不建议使用箭头函数

对于构造函数（类），一般定义时this指向新对象，不可定义为箭头函数

对于其他的函数，一般定义时指向调用者（没有就是global），箭头函数指向定义时指向定义时所在的对象或绑定定义时作用域

不适用箭头函数的情况：不可作为对象的方法，不可作为绑定事件的句柄
...
用const赋值某变量为引用类型的对象成员时，要特别注意此变量开始时不是引用类型的情况（undefined）
...
数组最大容量为 2 ** 32 -1，因为采用32位整型，且2 ** 32不是正常进制转换

# 2018-12-26

**javascript**

对象的键只可以是字符串或者Symbol，如果指定键不是这两者，会自动调用toString转换为字符串，如不能转换会报错

# 2018-12-28

**javascript**

模块之中，顶层的`this`关键字返回`undefined`，而不是指向`window`。也就是说，在模块顶层使用`this`关键字，是无意义的。CommonJS 模块的顶层`this`指向当前模块

模块中`export`通过接口，输出的是同一个值。不同的脚本加载这个接口，得到的都是同样的实例。而cjs则是不同的实例