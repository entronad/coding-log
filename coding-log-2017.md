...
const定义的变量必须立即赋值，否则报错

# 2017-11-23

**构建**

语义上的封装性：不能让调用代码不是依赖于类的公开接口，而是依赖于类的私用实现。每当你发现自己是通过查看类的内部实现来得知该如何使用这个类的时候，你就不是在针对接口编程了，而是在透过接口针对内部实现编程了。如果你透过接口来编程的话，封装性就被破坏了，而一旦封装性开始遭到破坏，抽象能力也就快遭殃了。
...
子程序的长度可达到100-200行，超过这个长度不会降低程序出错的风险，但会降低可读性

---

子程序参数排列原则：

- 按照输入-修改（既作为输入又作为输出）-输出的顺序排列参数
- 如果几个子程序都用了类似的参数，应该保持顺序一致
- 使用所有参数
- 把状态或出错变量放在最后
- 应在接口中对参数的假定进行说明，并用断言等进行校验
- 参数个数限制在7个以内
- 在命名上区分输入、修改、输出参数
...
把对子程序的调用和对状态值的判断清楚的分开。

# 2017-11-28

**构建**

断言通常只在开发与维护中使用，而不编译到产品中
...
隔栏（barricade）：把某些接口选定为“安全”区域的边界。对穿越安全区域边界的数据进行合法性校验，并当数据非法时做出敏锐的反映。让软件的某些部分处理“不干净的”数据，而让另一些部分处理“干净的”数据，即可让大部分代码无须再担负检查错误数据的职责
...
一般而言，绑定时间越早灵活性就会越差，但复杂性也会越低

---

变量名应该以问题为导向，反映领域问题本身，而不是解决使用的计算机术语

# 2017-11-29

**构建**

表示计算值的限定词（max，sum，total等）应当加在变量名的最后，总之原则是越重要的越要放前面

---

布尔变量，使用done，error，found，success等明确表示真/假的表语型的词，尽量用肯定形式，前面不要加is
...
- 在程序生命期中尽早决定国际化策略
- 如果只需要支持一种文字的语言，请考虑使用ISO 8859字符集
- 如果你需要支持多种语言，请使用Unicode
- 采用某种一致的字符串类型转换策略

布尔变量可以起到对程序加以说明和简化负责判断的作用

考虑使用容器（集合、栈、队列）来取代数组，或者将数组作为顺序化结构来处理，尽量不要直接按下标访问数组
...
每一个if, while, repeat, for, and, or, case或类似的控制关键字加1

子程序的复杂度保持在5以内，最好不要超过10
