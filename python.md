...
列表生成式：[x * x for x in range(1, 11) if x % 2 == 0]，[m + n for m in 'ABC' for n in 'XYZ']是两层循环
...
0、空集合（包括字符串）、None 都被认为是 False

---

max函数还可以以iterable为参数，还可以有一个key参数，用来获取所要比较的对象，比如
...
xrange与range合并为range，如果range的左右相等，将是空数组
...
前单：模块内部变量，类内部变量，这样的变量不会被import
